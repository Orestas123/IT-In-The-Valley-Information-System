...
import controller.issue.NewIssueController;
...
public class MagazineController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Platform.runLater(() -> {
            ...
            if (totalPatient == 0) {
                lblTotalPatient.setText("No magazines found");
    ...
    private void handlePrevButton(ActionEvent event) {
        if (paginate.getStart() > 0) {
            ...
            System.out.println("Magazine default : " + patientDefault);
            if (patientDefault) {
                loadPatients();
    ...
    private void loadPatients() {
        ...
        patients.forEach((patient) -> {
            MagazineCard card = new MagazineCard(patient) {
                @Override
    ...
    private void openPrescriptionStage(int patientId) {
        FXMLLoader fXMLLoader = new FXMLLoader(getClass().getResource("/view/issue/NewIssue.fxml"));
        try {
            Parent root = fXMLLoader.load();
            NewIssueController controller = fXMLLoader.getController();
            ...
            Logger.getLogger(MagazineController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    private void openPatientEditStage(int patientId) {
        FXMLLoader fXMLLoader = new FXMLLoader(getClass().getResource("/view/magazine/EditMagazine.fxml"));
        try {
            Parent root = fXMLLoader.load();
            EditMagazineController controller = fXMLLoader.getController();
    ...
    private void openPatientHistoryStage(Patient patient) {
        if (patient.getNumberOfPrescription() == 0) {
            ...
            alert.setTitle("There are no magazine issues");
            alert.setHeaderText("History not found");
            alert.setContentText("There are no issues to show");
            alert.showAndWait();
        } else {
            System.out.println("Go on");
            FXMLLoader fXMLLoader = new FXMLLoader(getClass().getResource("/view/magazine/MagazineHistory.fxml"));
            try {
                Parent root = fXMLLoader.load();
                MagazineHistoryController controller = fXMLLoader.getController();
    ...
    private void openDeleteDialog(Patient patient) {
        ...
        alert.setTitle("Delete magazine ?");
        ...
        if (result.get() == ButtonType.OK) {
            ...
            if (patientGetway.delete(patient.getId())) {
                ...
                nAlert.setHeaderText("Magazine has been deleted");
                nAlert.setContentText("Magazine Deleted");
                nAlert.show();
                if (patientDefault) {
                    loadPatients();
                } else {
