...
import controller.magazine.MagazineController;
...
public class AdsController implements Initializable {
    ...
    private void editDrug(Drug drug) {
        FXMLLoader fXMLLoader = new FXMLLoader(getClass().getResource("/view/ads/EditAds.fxml"));
        try {
            Parent root = fXMLLoader.load();
            EditAdsController controller = fXMLLoader.getController();
            ...
            stage.setTitle("Edit Advert/Media");
    ...
    private void deleteDrug(int drugId, String name) {
        ...
        alert.setContentText("Are you sure you want to delete this Ad/Media ?");
                    ...
                    Actions actions = new Actions() {
                        ...
                        protected void handleDeleteButton(ActionEvent actionEvent) {
                            if (drug.getUseInPrescription() > 0) {
                                ...
                                alert.setTitle("Advert is in Magazine");
                                alert.setHeaderText("Advert is used in Magazine");
                                alert.setContentText("This advert has been used by " + drug.getUseInPrescription() + " Prescription(s) \nYou cannot delete this drug");
                                alert.showAndWait();
                            } else if (drug.getUseInTemplate() > 0) {
                                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                                alert.setTitle("advert is in template");
                                alert.setHeaderText("Advert is used in template");
                                alert.setContentText("This advert has been used by " + drug.getUseInTemplate()+ " Template(s) \nYou cannot delete this drug");
                                alert.showAndWait();
                            } else {
