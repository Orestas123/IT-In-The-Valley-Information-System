...
public class NewIssueController implements Initializable {
    ...
    private void handlePrescriptonHistory(ActionEvent event) {
        System.out.println("Advert id : " + comboBoxHistory.getSelectionModel().getSelectedItem().getId());
    ...
    public void loadPatient(int patientId) {
        ...
        lblAge.setText("Duration : " + ChronoUnit.YEARS.between(birthdate, now) + "Days");
        lblSex.setText("Type :" + sex);
        showImage(patient);
    }
    ...
    private void loadDrug() {
        ...
        comboBoxDrug.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends Drug> observable, Drug oldValue, Drug newValue) -> {
            System.out.println("Selected ad id : " + newValue.getId() + " Name : " + newValue.getName());
        });
    }
    ...
    private void loadPrintablePrescription(int prescriptionId, boolean stageClose) {
        FXMLLoader fXMLLoader = new FXMLLoader(getClass().getResource("/view/issue/ViewIssue.fxml"));
        try {
            Parent root = fXMLLoader.load();
            ViewIssueController controller = fXMLLoader.getController();
            controller.loadPrescription(prescriptionId);
            Stage stage = new Stage();
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.setTitle("Print Magazine");
    ...
    private void deleteDrugFromList(TemplateDrug drug) {
        ...
        alert.setContentText("Click ok if you want to delete this ad");
    ...
    private void showImage(Patient patient) {
        ...
        if (patient.getThumbnail() == null) {
            imagePatientView.setImage(new Image(getClass().getResource("/image/magazine.jpg").toExternalForm()));
        } else {
